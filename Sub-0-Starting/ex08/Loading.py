...
def format_time(duration):
    """Takes a duration in time format and format it to display
        a min:sec

        Takes: Time object

        Return: Formated Time object"""
    minutes, seconds = divmod(int(duration), 60)
    return (f"{minutes:02d}:{seconds:02d}")


def ft_tqdm(lst: range) -> None:
    """Immitates the tdqm function
    ...
    try:
        ...
        print(f"OSError caught: {e} (Do not use cat -e)")
        exit(1)
